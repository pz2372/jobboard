...
class EmployerLocalStorageService {
  ...
  hasValidData(): boolean {
    try {
      ...
      const parsedData: EmployerLocalStorageData = JSON.parse(data);
      return (parsedData.jobs !== null || parsedData.applications !== null) && !this.isDataExpired(parsedData.lastUpdated);
    } catch (error) {
      return false;
    }
  }
  ...
  getDataAge(): number {
    try {
      ...
      const parsedData: EmployerLocalStorageData = JSON.parse(data);
      if (!parsedData.lastUpdated) return Infinity;
      
      const lastUpdated = new Date(parsedData.lastUpdated).getTime();
      const now = Date.now();
  ...
  private isDataExpired(lastUpdated: string): boolean {
    ...
    const now = Date.now();
    const dataAgeHours = (now - lastUpdatedTime) / (1000 * 60 * 60);
    return dataAgeHours > this.EXPIRY_HOURS;
  }
