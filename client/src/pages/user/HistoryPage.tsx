  ...
  useEffect(() => {
    const fetchHistoryData = async () => {
      try {
        ...
        {/*const cachedSavedJobs = localStorage.getItem('savedJobsData');
        const cachedAppliedJobs = localStorage.getItem('appliedJobsData');
        const cacheTimestamp = localStorage.getItem('historyDataTimestamp');
        const CACHE_DURATION = 24 * 60 * 60 * 1000; // 24 hours in milliseconds

        if (cachedSavedJobs && cachedAppliedJobs && cacheTimestamp && 
            (now - parseInt(cacheTimestamp)) < CACHE_DURATION) {
          // Use cached data if it's not expired
          setSavedJobs(JSON.parse(cachedSavedJobs));
          setAppliedJobs(JSON.parse(cachedAppliedJobs));
        } else {*/}
          // Fetch fresh data from server
          const [savedResponse, appliedResponse] = await Promise.all([
            axiosInstance.get(`/savedjobs/${user.id}`),
            axiosInstance.get(`/applications/user/${user.id}`)
          ]);
          ...
          localStorage.setItem('historyDataTimestamp', now.toString());
